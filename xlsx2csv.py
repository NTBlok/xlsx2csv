...
class Xlsx2csv:
    ...
    def __init__(self, xlsxfile, **options):
        ...
        options.setdefault("rows_w_fill", False)
    ...
    def _convert(self, sheetid, outfile):
        ...
        try:
            ...
            try:
                ...
                sheet.set_rows_w_fill(self.options['rows_w_fill'])
            ...
            for cellXfs in cellXfsElement[0].childNodes:
                ...
                if cellXfs._attrs and 'fillId' in cellXfs._attrs:
                    ...
                    if fillId > 1:
                        ...
                        containsfill = 0
                    self.cellXfs_fill.append(containsfill)
                else:
                    self.cellXfs_fill.append(None)
...
class Sheet:
    ...
    def set_rows_w_fill(self, rows_w_fill):
        self.rows_w_fill = rows_w_fill
    ...
    def handleRowFill(self, s):
        xfs_fill = self.styles.cellXfs_fill[s]
        if int(xfs_fill) > 1 and not self.row_fill:
            self.row_fill = True
    ...
    def handleEndElement(self, name):
        ...
        if self.in_row and (name == 'row' or (has_namespace and name.endswith(':row'))):
            if len(self.columns.keys()) > 0:
                ...
                # write line to csv
                if self.rows_w_fill and not self.row_fill:
                    write_row_1 = False
                else: 
                    write_row_1 = True

                write_row_0 = not self.skip_empty_lines or d.count('') != len(d)    
                if write_row_0 and write_row_1:
...
if __name__ == "__main__":
    ...
    try:  parser.add_argument("--ignore-formats", nargs=nargs_plus, type=str, dest="ignore_formats", default=[''],
    ...
    parser.add_argument("--highlight", "--highlight", dest="rows_w_fill", default=False, action="store_true",
      help="rows filled with highlight")
    ...
    kwargs = {
      ...
      'rows_w_fill' : options.rows_w_fill,
